...
When LOOP, GRAPH, or LOOP+GRAPH applies, load `loop-graph-system` with `skill_view` before planning or tool use. Use the simplest shape that can finish and verify the task. Never build orchestration for a simple one-shot.
